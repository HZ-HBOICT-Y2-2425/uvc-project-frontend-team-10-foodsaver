<script lang="ts">
    ...
    async function fetchRecipes(pantry) {
        ...
        try {
            const response = await fetch(
                `https://api.spoonacular.com/recipes/findByIngredients?ingredients=${ingredients}&number=8&apiKey=${apiKey}`,
            );
            if (response.ok) {
    ...
    onMount(async () => {
        ...
        try {
            ...
            const response = await fetch(
                `https://api.spoonacular.com/recipes/findByIngredients?ingredients=${seasonalIngredient}&number=8&apiKey=${apiKey}`,
            );
            if (response.ok) {
                const fetchedRecipes = await response.json();
