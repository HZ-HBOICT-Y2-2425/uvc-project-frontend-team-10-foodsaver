  ...
  onMount(async () => {
    try {
      const response = await fetch(
        "http://localhost:4001/api/users/co2-leaderboard",
      );
      if (response.ok) {
  ...
  <div class="podium">
    ...
    {#if leaderboard.length > 1}
      <div class="podium-item">
        <img
          class="medal"
          src={leaderboard[1].medal}
          alt={`Medal for rank ${leaderboard[1].rank}`}
        />
    ...
    {#if leaderboard.length > 0}
      <div class="podium-item center-podium">
        <img
          class="medal"
          src={leaderboard[0].medal}
          alt={`Medal for rank ${leaderboard[0].rank}`}
        />
    ...
    {#if leaderboard.length > 2}
      <div class="podium-item">
        <img
          class="medal"
          src={leaderboard[2].medal}
          alt={`Medal for rank ${leaderboard[2].rank}`}
        />
