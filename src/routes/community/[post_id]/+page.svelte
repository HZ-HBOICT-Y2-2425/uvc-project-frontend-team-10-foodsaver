<script lang="ts">
	...
	// Use onMount to fetch the recipe after component mount
	onMount(() => {
		// Subscribe to the page store to access the route params
		const unsubscribe = page.subscribe(($page) => {
			const { post_id } = $page.params; // Now, access $page.params correctly
			fetchRecipe(post_id); // Pass the post_id to the fetch function
		});
	...
	async function fetchRecipe(post_id) {
...
<div class="container mx-auto mt-6 p-6">
	{#if post}
		<div class="recipe-card border p-4 rounded-lg shadow-md">
			{#if isEditing}
				<!-- Edit Mode -->
				<h1 class="text-3xl text-green-700 font-bold mb-4">
					Edit Recipe
				</h1>
				<input
					type="text"
			...
			{:else}
				...
				<p class="text-lg text-gray-700 mt-2">{post.content}</p>
				<!-- Recipe details -->
