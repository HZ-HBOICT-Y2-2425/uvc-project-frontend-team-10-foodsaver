<script>
    ...
    async function fetchTips() {
      ...
      try {
        const response = await fetch(
          `http://localhost:3016/tips/makeuse`, // Asegúrate de que la URL sea correcta
          {
            method: 'GET',
            headers: {
        ...
        tips = await response.json(); // Asigna los tips obtenidos
        // if (response.ok) {
        //   tips = await response.json(); // Asigna los tips obtenidos
        // } else {
  ...
  <div>
    <div class="container">
      ...
      <!-- Botón para cargar los tips -->
      <button
        class="load-tips-button"
        on:click={fetchTips}  
        disabled={loading}   
      >
        {loading ? 'Loading tips...' : 'Load Tips'}
      </button>
