<script lang="ts">
    ...
    const API_KEY = '5aae41c17ad940b8a92e9f218de9bdb8'; // Replace with your actual API key
            ...
            const detailedRecipes = await Promise.all(
              data.map(async (recipe: any) => {
                ...
                if (!detailsResponse.ok) {
                  console.error(`Failed to fetch details for recipe ${recipe.id}`);
                  return null;
                }
  ...
  <div class="container mx-auto mt-6 p-6">
    ...
    <div class="filters mb-4">
      ...
      <label class="inline-flex items-center mr-4">
        <input type="checkbox" bind:checked={isVegetarian} on:change={applyFilters} />
        <span class="ml-2">Vegetarian</span>
      </label>
      <label class="inline-flex items-center">
        <input type="checkbox" bind:checked={isGlutenFree} on:change={applyFilters} />
    ...
    {#if filteredRecipes.length > 0}
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {#each filteredRecipes as recipe}
          <div class="recipe-card border p-4 rounded-lg text-center shadow hover:shadow-lg transition-all">
            <img src={recipe.image} alt={recipe.title} class="w-full h-32 object-cover rounded-md mb-2" />
